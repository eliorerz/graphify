...
def _make_graph() -> nx.DiGraph:
    """Build a small test graph mimicking OSAC structure."""
    G = nx.DiGraph()
    # fulfillment-service files
    G.add_node("fs_main", source_file="fulfillment-service/cmd/fulfillment-service/main.go", label="main.go")
    G.add_node("fs_clusters", source_file="fulfillment-service/internal/servers/clusters_server.go", label="clusters_server.go")
    G.add_node("fs_subnets", source_file="fulfillment-service/internal/servers/subnets_server.go", label="subnets_server.go")
    G.add_node("fs_proto", source_file="fulfillment-service/internal/api/osac/public/v1/clusters_service.pb.go", label="clusters_service.pb.go")
    G.add_node("fs_db", source_file="fulfillment-service/internal/database/clusters.go", label="clusters.go")
    G.add_node("fs_lint", source_file="fulfillment-service/dev/lint.py", label="lint.py")
    G.add_node("fs_chart", source_file="fulfillment-service/charts/values.yaml", label="values.yaml")
    G.add_node("fs_it", source_file="fulfillment-service/it/integration_test.go", label="integration_test.go")

    # osac-operator files
    G.add_node("op_ctrl", source_file="osac-operator/internal/controller/cluster_controller.go", label="cluster_controller.go")
    G.add_node("op_api", source_file="osac-operator/api/v1alpha1/cluster_types.go", label="cluster_types.go")
...
class TestFindNodesForFile:
    def test_find_with_repo_prefix(self):
        G = _make_graph()
        nodes = find_nodes_for_file(G, "internal/servers/clusters_server.go", "fulfillment-service")
        assert "fs_clusters" in nodes

    def test_find_no_match(self):
        G = _make_graph()
        nodes = find_nodes_for_file(G, "nonexistent_file.go", "fulfillment-service")
        assert nodes == []
...
class TestCiSelect:
    ...
    def test_cross_repo_detection(self, tmp_path):
        ...
        assert "osac-operator" in cross_repos
...
class TestTestPlan:
    def test_to_dict(self):
        plan = TestPlan(
            ...
            cross_repo=[{"repo": "osac-operator", "tests": ["check-generated-code"]}],
        ...
        assert d["test_plan"]["cross_repo"][0]["repo"] == "osac-operator"
